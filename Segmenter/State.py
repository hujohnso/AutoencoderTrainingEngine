...
class State:
    ...
    def update_state(self, segmented_image_for_update):
        ...
        self.state_object_set = self.map_single_image_output_to_state_objects(set_of_numbered_object_labels, self.current_image, self.state_object_set)
    ...
    def map_single_image_output_to_state_objects(self, set_of_numbered_object_labels, segmented_image, set_of_current_state_objects):
        new_state_objects = self.create_new_set_of_state_objects_from_set_of_ids(set_of_numbered_object_labels, segmented_image)
        ...
        return self.map_new_state_object_indices_to_current_state_objects(chosen_indicies, new_state_objects, set_of_current_state_objects)
        ...
        for new_state_object_index in range(len(set_of_new_state_objects)):
            for current_state_object_index in range(len(set_of_current_state_objects)):
                probability_matrix[new_state_object_index,current_state_object_index] = \
                    StateObjectHelpers.StateObjectHelpers.get_centroid_distance(set_of_new_state_objects[new_state_object_index], set_of_current_state_objects[current_state_object_index])
            ...
            for current_state_object_index in range(probability_matrix.shape[1]):
                if minimum_centroid_distance is not None and probability_matrix[new_state_object_index, current_state_object_index] < minimum_centroid_distance:
                    if minimum_centroid_index not in chosen_indices:
                        ...
                        minimum_centroid_distance = probability_matrix[new_state_object_index, current_state_object_index]
            chosen_indices.append(minimum_centroid_index)
        return chosen_indices

    def map_new_state_object_indices_to_current_state_objects(self, chosen_indices, list_of_new_state_objects, list_of_current_state_objects):
