...
class SmallConvolutionalAutoEncoderSlightlyDifferent(AutoEncoder):
    ...
    def create_autoencoder(self, input_image_vector):
        ...
        encoded = Conv2D(256, kernel_size=(3, 3), activation='linear', padding='same', kernel_initializer=self.get_initializer())(input_image)
        encoded = MaxPooling2D(pool_size=(3, 3), padding='same')(encoded)
        encoded = Conv2D(128, (3, 3), activation='linear', padding='same', kernel_initializer=self.get_initializer())(encoded)
        encoded = MaxPooling2D(pool_size=(3, 3), padding='same')(encoded)
        ...
        encoded = Conv2D(64, (3, 3), activation='linear',padding='same', kernel_initializer=self.get_initializer())(encoded)
        encoded = MaxPooling2D(pool_size=(3, 3), padding='same')(encoded)
        encoded = Conv2D(32, (3, 3), activation='linear',padding='same', kernel_initializer=self.get_initializer())(encoded)
        encoded = MaxPooling2D(pool_size=(3, 3), padding='same')(encoded)
        encoded = Conv2D(16, (3, 3), activation='linear',padding='same', kernel_initializer=self.get_initializer())(encoded)
        encoded = MaxPooling2D(pool_size=(3, 3), padding='same')(encoded)
